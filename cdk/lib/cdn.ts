...
import { Api } from './api';
import { Waf } from './waf';
import { StaticSiteProps } from './static-site-stack';
...
export class Cdn extends Construct {
  ...
  private createViewerRequestFunction() {
    ...
    const staticRewriteFunctionCode = readFileSync(
      path.join(__dirname, '../cloudfront/viewer-request.js'), 'utf8'
    )
      .replace('KEY_VALUE_STORE_ID_PLACEHOLDER', this.keyValueStore.keyValueStoreId,)
  ...
  private createDistribution() {
    return new Distribution(this, 'Distribution', {
      ...
      defaultBehavior: {
        origin: S3BucketOrigin.withOriginAccessControl(this.props.objectStore.bucket),
  ...
  private addApiBehavior() {
    const apiOrigin = new HttpOrigin(`${this.props.api?.restApi.restApiId}.execute-api.${Stack.of(this).region}.${Stack.of(this).urlSuffix}`, {
      customHeaders: {
        'x-api-key': this.props.api?.apiKeySecret.secretValue.toString() ?? 'secret',
      },
