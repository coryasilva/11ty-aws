...
test("Defaults: no API, no WAF, yes Deployment", () => {
  ...
  const noWaf =
    Object.keys(template.findResources("AWS::WAFv2::WebACL")).length === 0;
  const noApi =
    Object.keys(template.findResources("AWS::ApiGateway::RestApi")).length ===
    0;
...
test("WAF enabled", () => {
  const app = new cdk.App();
  const stack = new StaticSiteStack(app, "WafEnabled", {
    ...mockProps,
    wafEnabled: true,
    apiEnabled: true,
  });
...
test("API enabled", () => {
  const app = new cdk.App();
  const stack = new StaticSiteStack(app, "ApiEnabled", {
    ...mockProps,
    apiEnabled: true,
  });
  const template = Template.fromStack(stack);
