...
const viewerRequestCode = readFileSync(
  path.join(__dirname, './viewer-request.js'),
).toString();
...
function viewerReqest(event) {
  eval(
    viewerRequestCode.replace(/^\s*import cf from 'cloudfront'.*$/im, mockCf),
  );
...
describe('Viewer-request cloudfront function', () => {
  ...
  it('should redirect to canonical url', async () => {
    ...
    expect((await viewerReqest(mock)).headers.location.value).toBe(
      'https://CANONICAL_HOST/page/',
    );
